...
from root.mlp.utils import get_model, compute_training_scores, append_to_stats
...
class Regressor:
    ...
    def kfold(self, dataframe, epochs= EPOCHS, k= K):
        preprocessed_data = dataframe.copy()
        year = None
        if len(set(preprocessed_data['year'].values))==1:
            year = preprocessed_data.pop('year').values[0]
        ...
        append_to_stats(stats, self.stats)

        return stats

    def holdout(self, train_dataframe, test_dataframe, epochs=EPOCHS):
        train_df = train_dataframe.copy()
        test_df  = test_dataframe.copy()
        year = None
        if len(set(pd.concat([test_df, train_df])['year'].values))==1:
            test_df.pop('year')
            years = train_df.pop('year')
            year = years.values[0]
